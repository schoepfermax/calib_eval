...
from DatasetLidarCamera import DatasetLidarCameraKittiOdometry, DatasetLidarCameraThesisStatic
...
def config():
    ...
    dataset = 'thesis/static'
    data_folder = '/home/khan/dataset_root/static_mount_h1'
    reference_path = '/home/khan/calib_ws/src/calib_eval/config/reference/static_mount_h1_reference.yaml'
    ...
    stage = 'sanity'
    smoke_test = False
    experiment_name = 'lccnet_h1'
    experiment_log_csv = './checkpoints/training_runs_log.csv'
    epochs = 3
...
def _append_experiment_row(csv_path, row):
    fieldnames = [
        'timestamp', 'status', 'model', 'dataset', 'split', 'stage',
        'reference_extrinsic_file', 'perturbation_range', 'checkpoint',
        'train_loss', 'val_metric', 'translation_error', 'rotation_error',
        'notes', 'output_dir'
    ]
    _safe_makedirs(os.path.dirname(csv_path) or '.')
...
def _prepare_batch_tensors(sample, img_shape, max_depth):
    lidar_input = []
    rgb_input = []
    lidar_gt = []
    shape_pad_input = []
    ...
    for idx in range(len(sample['rgb'])):
        ...
        depth_gt, _ = lidar_project_depth(pc_lidar, sample['calib'][idx], real_shape)
        depth_gt /= max_depth
        ...
        depth_img, _ = lidar_project_depth(pc_rotated, sample['calib'][idx], real_shape)
        depth_img /= max_depth
...
def _run_smoke_test(model, optimizer, loss_fn, train_loader, config, model_savepath):
    print("Running smoke test: one batch, one forward pass, one loss, one checkpoint save.")
    first_batch = next(iter(train_loader))
    prepared = _prepare_batch_tensors(first_batch, img_shape=(384, 1280), max_depth=config['max_depth'])
    loss, _, _ = train(
        model,
        optimizer,
        prepared['rgb_input'],
        prepared['lidar_input'],
        prepared['sample']['tr_error'],
        prepared['sample']['rot_error'],
        loss_fn,
        prepared['sample']['point_cloud'],
        config['loss'],
    )
    smoke_ckpt = os.path.join(model_savepath, 'smoke_test_checkpoint.tar')
    torch.save({
        'config': config,
        'smoke_test': True,
    ...
    print(f"Smoke test passed. Checkpoint saved to {smoke_ckpt}")
    return smoke_ckpt, loss['total_loss'].item()
...
def val(model, rgb_img, refl_img, target_transl, target_rot, loss_fn, point_clouds, loss):
    ...
    with torch.no_grad():
        ...
        target_rot = target_rot.to(rot_err.device)
    ...
    total_trasl_error = torch.tensor(0.0, device=transl_err.device)
    target_transl = target_transl.to(transl_err.device)
    target_rot = target_rot.to(rot_err.device)
...
def main(_config, _run, seed):
    ...
    run_name = f"{_config['experiment_name']}_{_config['stage']}_{timestamp}"

    if _config['dataset'] == 'thesis/static':
        dataset_class = DatasetLidarCameraThesisStatic
        dataset_train = dataset_class(
            ...
            split_file=_config['train_split_file'],
            max_samples=_config['max_train_samples'],
        )
        dataset_val = dataset_class(
            ...
            use_reflectance=_config['use_reflectance'],
            reference_path=_config['reference_path'],
            split_file=_config['val_split_file'],
            max_samples=_config['max_val_samples'],
        )
        checkpoints_dir = os.path.join(_config["checkpoints"], 'thesis_static', run_name)
        model_savepath = os.path.join(_config['checkpoints'], 'models')
        log_savepath = os.path.join(_config['checkpoints'], 'log')
    else:
    ...
    _safe_makedirs(model_savepath)
    _safe_makedirs(log_savepath)
    train_writer = SummaryWriter(os.path.join(log_savepath, 'train'))
    val_writer = SummaryWriter(os.path.join(log_savepath, 'val'))

    _append_experiment_row(_config['experiment_log_csv'], {
        'timestamp': timestamp,
        'status': 'started',
        'model': 'LCCNet',
        'dataset': _config['data_folder'],
        'split': 'train/val',
        'stage': _config['stage'],
        'reference_extrinsic_file': _config['reference_path'],
        'perturbation_range': f"t={_config['max_t']}, r={_config['max_r']}",
        'checkpoint': _config['weights'],
        'train_loss': '',
        'val_metric': '',
        'translation_error': '',
        'rotation_error': '',
        'notes': run_name,
        'output_dir': _config['checkpoints'],
    })
    ...
    if _config['weights'] is not None and os.path.exists(_config['weights']) and _config['resume']:
        checkpoint = torch.load(_config['weights'], map_location='cpu')
    ...
    if _config['smoke_test']:
        smoke_ckpt, smoke_loss = _run_smoke_test(model, optimizer, loss_fn, TrainImgLoader, _config, model_savepath)
        _append_experiment_row(_config['experiment_log_csv'], {
            'timestamp': timestamp,
            'status': 'smoke_passed',
            'model': 'LCCNet',
            ...
            'train_loss': smoke_loss,
            'val_metric': '',
            'translation_error': '',
            'rotation_error': '',
            'notes': 'Smoke test completed successfully',
            'output_dir': _config['checkpoints'],
        })
        return smoke_loss
    ...
    _append_experiment_row(_config['experiment_log_csv'], {
        ...
        'reference_extrinsic_file': _config['reference_path'],
        'perturbation_range': f"t={_config['max_t']}, r={_config['max_r']}",
        'checkpoint': old_save_filename if old_save_filename is not None else _config['weights'],
        'train_loss': total_train_loss / len(dataset_train),
        'val_metric': total_val_loss / len(dataset_val),
        'translation_error': total_val_t / len(dataset_val),
        'rotation_error': total_val_r / len(dataset_val),
        'notes': run_name,
        'output_dir': _config['checkpoints'],
    })
    return _run.result
