...
def gen_dx_bx(xbound, ybound, zbound):
    ...
    # Bounds are stored as floats in bev_settings.py. The voxel counts must be
    # explicit integers before constructing a LongTensor.
    nx_vals = [
        int(round((row[1] - row[0]) / row[2]))
        for row in [xbound, ybound, zbound]
    ]
    nx = torch.LongTensor(nx_vals)
    return dx, bx, nx
